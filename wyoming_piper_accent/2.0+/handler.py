...
def preprocess_text_for_stress(text: str, accentor: Optional[Any], user_marker: str = '+') -> str:
    ...
    if user_marker in current_text:
        ...
        for part in parts:
            if not part or part.isspace() or any(c in _PUNCTUATION for c in part if not c.isalnum()):
...
class PiperEventHandler(AsyncEventHandler):
    def __init__(self, wyoming_info: Info, cli_args: argparse.Namespace,
                 voices_info: Dict[str, Any], accentor: Optional[Any],
                 *args, **kwargs) -> None:
    ...
    async def _process_and_synthesize(self, sentence: str, synthesize_obj: Synthesize):
        ...
        if sentence in ("<PARAGRAPH_BREAK>", "<DIALOGUE_BREAK>"):
            ...
            await self._handle_synthesize(synthesize_obj, send_stop=False, break_type=b_type)
            return

        if not sentence.strip():
            return

        # 2. Фикс для Home Assistant (если HA прислал новую реплику отдельным событием без \n)
        stripped_sentence = sentence.lstrip()
        if self._is_first_sentence_in_event and stripped_sentence.startswith(('—', '–')):
            await self._handle_synthesize(synthesize_obj, send_stop=False, break_type="dialogue")
        
        self._is_first_sentence_in_event = False

        # 3. Выбор голоса и определение языка для нормализации
        ...
        # 5. Буферизация предложений до лимита
    ...
    async def _handle_synthesize(self, synthesize: Synthesize, send_stop: bool = True,
                                 break_type: Optional[str] = None) -> bool:
        ...
        voice_name, voice_speaker = (
            ...
            if synthesize.voice else (None, None)
        )
        ...
        async with _VOICE_LOCK:
            ...
            if self.cli_args.sentence_silence > 0:
                silence_sec = random.uniform(
                    self.cli_args.sentence_silence * 0.6,
