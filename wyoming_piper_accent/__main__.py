...
async def main() -> None:
    ...
    parser.add_argument(
        "--no-automatic-stress",
        action="store_true",
        help="Disable and do not load the automatic stress placement model", # Обновили help
    )
    ...
    if not args.no_automatic_stress:
        ...
        try:
            ...
            accentor = None # Убедимся, что None в случае ошибки
    else:
    ...
    def _handle_shutdown(sig, frame):
        _LOGGER.info("Received signal %s, shutting down......", signal.strsignal(sig))
        shutdown_event.set()
    ...
    handler_factory = partial(
        ...
        args,
        process_manager,
        accentor, # Передаем загруженную модель (или None)
    )
    ...
    if hasattr(process_manager, 'shutdown'):
        await process_manager.shutdown()
    else:
        _LOGGER.warning("PiperProcessManager does not have a shutdown method. Processes may be orphaned.")
