...
class PiperProcessManager:
    ...
    def __init__(self, args: argparse.Namespace, voices_info: Dict[str, Any]):
        ...
        self.processes_lock = asyncio.Lock() # Этот лок не используется, можно убрать
                                             # Если будет использоваться, нужно вернуть
    ...
    # <<< НОВОЕ: Метод для грациозного завершения всех дочерних процессов >>>
    async def shutdown(self):
        """Terminates all running piper processes."""
        _LOGGER.info("Stopping %s piper process(es)...", len(self.processes))
        
        # Создаем копию списка процессов для итерации, так как мы будем их удалять
        running_procs = list(self.processes.values())
        self.processes.clear()

        termination_tasks = []
        for piper_proc in running_procs:
            if piper_proc.proc.returncode is None:
                # Если процесс еще работает
                _LOGGER.debug(
                    "Terminating process for voice '%s' (pid=%s)",
                    piper_proc.name,
                    piper_proc.proc.pid,
                )
                # Отправляем сигнал на завершение и создаем задачу для ожидания
                piper_proc.proc.terminate()
                termination_tasks.append(piper_proc.proc.wait())

        if termination_tasks:
            # Ожидаем завершения всех процессов параллельно
            await asyncio.gather(*termination_tasks, return_exceptions=True)

        _LOGGER.info("All piper processes have been stopped.")